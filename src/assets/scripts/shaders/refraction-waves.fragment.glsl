...
void main(void) {
    ...
    // vec2(vUv.s + 0.015, vUv.t + 0.015)) 0.015 искажение
    // смещение
    float distortionOffest = 0.005;
    vec4 tx = texture2D( img1, vec2(vUv.s + distortionOffest, vUv.t + distortionOffest)) * texture2D( img1, vec2( vUv.s + cos(c) * mouse.x * 0.5, vUv.t + cos(c) * mouse.y * 0.5 ) ) * 0.25;
